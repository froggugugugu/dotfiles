...
set foldlevel=100       " 折り畳みはマーカーで実施
set commentstring=//%s 
" ステータスライン
set laststatus=2
set statusline=%<%F
...
" ===================== その他の設定 ====================="
" 環境変数の設定
let $WORK = $HOME."/Documents"
let $SNIPPET = $HOME."/.vim/snippets"
...
" スニペットファイルの場所
let g:neocomplcache_snippets_dir = $SNIPPET
" 1番目の候補を自動選択
...
" スニペット編集
command! -nargs=* Nes NeoComplCacheEditSnippet <args>
...
" 選択部分をクリップボードの値に置き換え
vmap <C-V> d"*P
" コマンドライン時、クリップボードから貼り付け(制御コードが入力できなくなる)
"cmap <C-V> <C-R>*
" 選択部分をクリップボードに切り取り
vmap <C-X> "*d<ESC>
" ハイライトを消す
